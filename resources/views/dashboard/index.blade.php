...
@section('content')
    <section>
        <div class="flex justify-between align-center">
            ...
            <div>
                <div class="mb-4">
                    <a href="/dashboard/audience" class="btn-primary" type="submit">Audience Stats</a>
                </div>
            </div>
        </div>
        <section class="my-4">
        ...
        <section>
            ...
            @if ($posts->count())
                @foreach ($posts as $post)
                    <div class="py-5 border-b border-gray-200 flex gap-3">
                        <p class="p-0 m-0">{{ $post->content }}</p> <span><b>({{ $post->likes->count() }}
                                {{ Str::plural('like', $post->likes->count()) }})</b></span>
                    </div>
                @endforeach
